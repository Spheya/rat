...
namespace ImLightRig {
	...
	inline int Edit(Light* lights, int lightCount, int selected = -1, ImVec2 size = ImVec2(0, 0)) {
		...
		ImVec2 frameSize((size.x < 0.001f) ? sz : size.x, (size.y < 0.001f) ? sz : size.y);
		...
		draw_list->AddLine(
		    ImVec2(canvas_pos.x + controlWidth * 0.5f, canvas_pos.y),
		    ImVec2(canvas_pos.x + controlWidth * 0.5f, canvas_pos.y + controlWidth),
		    0xFFAAAAAA
		);
		draw_list->AddLine(
		    ImVec2(canvas_pos.x, canvas_pos.y + controlWidth * 0.5f),
		    ImVec2(canvas_pos.x + controlWidth, canvas_pos.y + controlWidth * 0.5f),
		    0xFFAAAAAA
		);
		...
		for(int i = 0; i < lightCount; i++) {
			...
			const auto center(ImVec2(
			    canvas_pos.x + controlWidth * 0.5f + light.x * controlWidth * 0.5f, canvas_pos.y + controlWidth * 0.5f + light.y * controlWidth * 0.5f
			));
			...
			if(selected == i) { draw_list->AddCircle(center, radius * 1.5f, intensity, 0, 2.f); }
		...
		if(!io.MouseDown[0]) { movingLight = -1; }
